...
3. **레벨독해 변형 추가 전사** — 13강 변형 1~5회는 완료(L1~L4 채움). 남은 소스: 15강·17강(변형 1~5회)·19강·엘리트 6~10회. **방법**: 노션 회차 fetch → `_build/build_variants.py`의 `SETS`에 `{set_id, level, unit, source, title, items:[(유형,stem,lead,[선지5],정답0base,해설),…]}` 추가 → `python3 _build/build_variants.py && python3 _build/qc_check.py`. 난도매핑 1·2회=L1/3회=L2/4회=L3/5회+엘리트=L4. 회차당 20문항이 reading 세트+`var-*` drill로 동시 생성됨. OCR 오타 보정 필수.
