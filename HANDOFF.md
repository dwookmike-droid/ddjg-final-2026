...
> ⚠️ **백엔드 재배포 1건 대기.** 프론트는 v19까지 배포됨. v17 레벨독해 재배치·가이드, v18 학습 가이드 탭, **v19 진도 동기화 수정(재진입 시 서버 병합·종료 flush·로그인 병합)**. v19의 "재진입 서버 재동기화"는 `Code.gs`에 추가한 **`getProgress` 액션을 Apps Script에 붙여넣고 재배포**해야 완전 작동(미재배포 시 프론트는 graceful degradation=로컬만 사용).
...
- **16차(v19)** 진도 유실 수정: 재진입 시 서버 진도 병합(`Store.mergeProgress`+`API.getProgress`), 종료 시 즉시 flush(`API.flushNow`, sendBeacon, visibilitychange/pagehide), 로그인 덮어쓰기→병합. **백엔드 `getProgress` 재배포 필요**(Code.gs 반영됨).
