...
작성 2026-06-22(갱신 2026-06-22 6차). **새 세션은 이 파일을 먼저 읽고** 이어서 진행할 것. **다음 작업 = 배포 준비**(아래 §3-1).
...
## 0. 한 줄 상태
학생용 인터랙티브 최종점검 웹앱(반응형 PWA). **콘텐츠·기능·변형 전사(19강까지) 완성. git 초기화(main)·로컬 v7 검증 완료. 다음 세션은 배포 준비부터 시작** — 남은 건 사용자 손이 필요한 GitHub Pages 푸시·Apps Script·카카오 인증. (엘리트 6~9회 전사는 보류, 선택 작업.) 시험일 **2026-07-03(금)**.
...
## 3. 다음 할 일 (우선순위)
1. **배포 (최우선 · 다음 세션의 주 작업)** — git 초기화·12+커밋 완료(브랜치 `main`). **새 세션 진입 순서**: (a) `python3 _build/qc_check.py` 통과·`git status` 확인(클린) → (b) 사용자에게 GitHub 빈 저장소 생성 요청 후 `git remote add origin <url> && git push -u origin main` 안내 → (c) Settings·Pages(main/`/(root)`)에서 URL 확보, 학생 카톡 발송. (a)~(c)만으로 **로컬 모드 전체 동작**(카톡·동기화만 빠짐). 이후 선택: ② Apps Script(`initSheet`→웹앱 배포→`config.js`의 `APPS_SCRIPT_URL` 채우고 재푸시) ③ 카카오(REST키·`getKakaoAuthUrl` 1회 인증). 절차 상세는 [SETUP.md](SETUP.md). 내가 할 수 있는 건 (a)·안내, 계정/인증은 사용자만.
