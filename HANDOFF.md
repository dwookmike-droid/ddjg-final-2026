...
> ⚠️ **로컬 v18 미배포(푸시 안 됨).** 라이브는 아직 v16. 미배포 변경: ① 레벨독해 **지문↔문항 재배치 + 지문별 가이드 + 크로스 섹션**(reading.json 재매핑, v17) ② **학습 가이드 탭**(StudyGuide, v18). 배포는 캐시 확인 후 `git push`.
...
- **15차(로컬·미배포 v18)** 학습 가이드 탭(StudyGuide: 로드맵·오늘추천·사용법, 첫진입 자동노출). 원어민 TTS는 아카이빙(§3-1).
