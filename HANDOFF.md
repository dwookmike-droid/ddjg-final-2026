...
- **L1~L4 막다른 길 폴리시**: 빈 레벨 카드를 학생용으로 교체(「유형별 훈련으로 가기」 버튼 = `Router.go('drill')`). reading.js 수정 → **캐시 v5→v6**(index.html `?v=` + sw.js `VER` 동반). preview end-to-end 검증·콘솔 에러 0.
...
3. **레벨독해 L1~L4 탭** — 막다른 길 폴리시 적용됨(빈 탭 → 「유형별 훈련으로」 버튼). 실제 세트로 채우려면 노션 변형 1~5회(13·15·17)·19강·엘리트 6~10회를 난도별 reading 세트로 전사(OCR 오타 보정). **선택 작업.**
