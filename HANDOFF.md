...
> **현재 v21.** v17 레벨독해 재배치·가이드 · v18 학습 가이드 탭 · v20 진도 유실 수정(재진입 시 저장 PIN으로 silent 재로그인→병합, 종료 sendBeacon flush, 로그인 병합) · **v21 선생님 대시보드**(`?admin`).
> ⚠️ **선생님 대시보드는 백엔드 1회 작업 필요**: `Code.gs`에 `admin` 액션 추가됨 → Apps Script에 ① **스크립트 속성 `ADMIN_KEY` 설정**(`setAdminKey()` 편집 후 실행 or 속성 직접 입력) ② **재배포**. 그래야 `?admin` 대시보드가 데이터를 읽음(학생 앱은 재배포 없이 동작).
...
- **17차(v21)** 선생님 대시보드(`js/admin.js` `Admin`): `?admin` 진입→관리자 키→전체 학생 응시·평균·코스완료·최근활동·반별 통계·정렬. 백엔드 `apiAdmin`(key=ADMIN_KEY 검증, students+results 반환). **ADMIN_KEY 설정+재배포 필요.**
